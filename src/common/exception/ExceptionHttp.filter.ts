...
export class ExceptionHttpFilter implements ExceptionFilter {
    ...
    if (exception instanceof ValidateException) {
      ...
      const encryptedData = encryptAES_Hex(JSON.stringify(res), iv, process.env.AES_KEY)
      return response.status(code).send(encryptedData)
    }
    ...
    if (exception instanceof MuExceptions205) {
      ...
      return response.status(200).send(encryptedData)
    }
